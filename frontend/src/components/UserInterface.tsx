...
function UserInterface({ backendName }: UserInterfaceProps) {
  ...
  // Define styled based on the backend name
  ...
  const btnColor = buttonColor[backendName as keyof typeof buttonColor] || 'bg-gray-500 hover:bg-gray-600'
  ...
  const handleUpdateUser = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setUsers(
        users.map((user) => {
          if (user.id === Number.parseInt(updateUser.id)) {
      ...
      <div className="space-y-4">
        {users.map(user => (
          <div key={user.id} className="flex items-center justify-between bg-white p-4 rounded-lg shadow">
            ...
            <button type="button" onClick={() => deleteUser(user.id)} className={`${btnColor} text-white py-2 px-4 rounded`}>
              Delete User
            </button>
          </div>
        ))}
